...
// 判断整数 n 是否是素数 -> Kiểm tra số nguyên n có phải số nguyên tố không
...
Nói trước **nếu bạn muốn判断 một số có phải số nguyên tố không, nên viết thuật toán thế nào**. Chỉ cần sửa một chút điều kiện vòng for trong code `isPrime` ở trên:
...
Có thể thấy, hai tích sau chính là hai tích trước đảo ngược lại, điểm临界 đảo chiều nằm ở `sqrt(n)`.
...
Quay lại正题, ý tưởng cốt lõi của sàng số nguyên tố đi ngược với思路 thông thường ở trên:
...
class Solution {
    public int countPrimes(int n) {
        ...
        // 将数组都初始化为 true -> Khởi tạo mảng toàn true
        Arrays.fill(isPrime, true);

        for (int i = 2; i < n; i++) {
            if (isPrime[i]) {
                // i 的倍数不可能是素数了 -> Bội của i không thể là số nguyên tố nữa
...
Nếu hiểu được đoạn code trên, tức là bạn đã nắm思路 tổng thể, nhưng còn hai chi tiết nhỏ có thể tối ưu.

Trước hết, nhớ lại hàm判定 số nguyên tố `isPrime` giới thiệu ở đầu bài, do tính đối xứng của ước, vòng for trong đó chỉ cần duyệt `[2,sqrt(n)]` là đủ. Ở đây cũng tương tự, vòng for ngoài của chúng ta cũng chỉ cần duyệt tới `sqrt(n)`:
...
| - | [剑指 Offer 49. Số xấu](https://leetcode.cn/problems/chou-shu-lcof/?show=1) | 🟠 |
