...
Không biết mọi người làm bài thuật toán có cảm nhận gì, mình đúc kết mẹo làm bài thuật toán là: thu nhỏ bài toán lớn về một điểm, nghiên cứu trước cách giải quyết tại điểm nhỏ đó, rồi mở rộng ra toàn bộ bài toán bằng đệ quy/lặp.

Ví dụ như ở bài trước [Dẫn bạn quét cây nhị phân kỳ 3](https://labuladong.online/algo/data-structure/binary-tree-part3/), khi giải bài cây nhị phân, ta sẽ thu nhỏ toàn bộ vấn đề về một node nào đó, tưởng tượng mình đang đứng tại node đó thì cần làm gì, rồi áp khung đệ quy cây nhị phân vào là xong.

Dạng bài quy hoạch động (DP) cũng vậy, nhất là các bài liên quan đến dãy con (subsequence). **Bài này xuất phát từ「Bài toán dãy con chung dài nhất」và tổng kết ba bài toán dãy con**, giải kỹ bài này về mô-típ dạng bài dãy con, bạn sẽ cảm nhận được cách tư duy này.
...
Nếu chưa làm bài này bao giờ, một thuật toán brute-force đơn giản nhất là liệt kê tất cả các dãy con (subsequence) của `s1` và `s2`, rồi xem có dãy nào chung không, rồi trong tất cả các dãy con chung lại tìm một dãy dài nhất.

Rõ ràng, cách nghĩ này có độ phức tạp rất cao, bạn phải liệt kê mọi dãy con, độ phức tạp này là hàm mũ, chắc chắn không thực tế.

ý tưởng đúng là đừng xét cả chuỗi, mà hãy thu nhỏ về từng ký tự của `s1` và `s2`. Một quy luật đã tổng kết trong bài trước [Template giải bài toán dãy con](https://labuladong.online/algo/dynamic-programming/subsequence-problem/):





**Với bài toán tìm dãy con trên hai chuỗi, đều dùng hai con trỏ `i` và `j` di chuyển lần lượt trên hai chuỗi, xác suất lớn là ý tưởng quy hoạch động**.

Bài toán dãy con chung dài nhất cũng tuân theo quy luật này, ta có thể viết trước một hàm `dp`:
...
Ta chỉ nhìn `s1[i]` và `s2[j]`, **nếu `s1[i] == s2[j]`, ký tự này chắc chắn nằm trong `lcs`**:
...
**`s1[i] != s2[j]` có nghĩa là, trong `s1[i]` và `s2[j]` ít nhất có một ký tự không nằm trong `lcs`**:
...
Tới đây đã rất gần đáp án cuối cùng rồi, **còn một tối ưu nhỏ, trường hợp 3「cả `s1[i]` và `s2[j]` đều không nằm trong lcs」thật ra có thể bỏ qua trực tiếp**.

Vì ta đang tìm giá trị lớn nhất mà, trường hợp 3 tính độ dài `lcs` của `s1[i+1..]` và `s2[j+1..]`, độ dài này chắc chắn nhỏ hơn hoặc bằng độ dài `lcs` trong trường hợp 2 là `s1[i..]` và `s2[j+1..]`, vì `s1[i+1..]` ngắn hơn `s1[i..]` mà, thì `lcs` tính ra từ đó đương nhiên không thể dài hơn.
...
ý tưởng trên hoàn toàn đi theo bài nổi tiếng trước đây của ta là [Khung mô-típ quy hoạch động](https://labuladong.online/algo/essential-technique/dynamic-programming-framework/), hẳnlà rất dễ hiểu. Còn vì sao phải thêm `memo` bảng ghi nhớ, trước đây ta đã viết nhiều lần, để chăm sóc bạn đọc mới tới, ở đây nhắc lại đơn giản một chút, trước hết trừu tượng hóa khung đệ quy của hàm `dp` cốt lõi:
...
Tới đây, bài toán dãy con chung dài nhất đã được giải triệt để, dùng ý tưởng quy hoạch động top-down kèm bảng ghi nhớ, đương nhiên ta cũng có thể dùng ý tưởng quy hoạch động bottom-up dạng lặp, giống hệt ý tưởng đệ quy của ta, mấu chốt là định nghĩa mảng `dp` thế nào, mình cũng viết luôn cách giải bottom-up ở đây:
...
cách giải bottom-up trong định nghĩa mảng `dp` có hơi khác với cách giải đệ quy của ta, nhưng ý tưởng hoàn toàn giống cách giải đệ quy, nếu bạn hiểu cách giải đệ quy thì cách giải này hẳn không khó hiểu.
...
Ở vòng lặp thứ `i`, thứ bị thay đổi là giá trị `dp[i]`, lúc này theo định nghĩa, thứ cần so sánh là **ký tự thứ i** của `s1`, tức là `s1[i-1]`. `s2[j-1]` cũng tương tự. Chi tiết này rất hay gặp trong DP dạng chuỗi, cầnlưu ý thêm.

Ngoài ra, cách giải bottom-up có thể tối ưu bằng [Kỹ thuật nén không gian DP](https://labuladong.online/algo/dynamic-programming/space-optimization/) đãnóitrong bài trước, nén độ phức tạp không gian xuống O(N), ở đây vì giới hạn độ dài nên không mở rộng.
...
Cho hai từ `s1` và `s2`, trả về số bước tối t hiểu để khiến `s1` và `s2` giống nhau. Mỗi bước có thể xóa một ký tự bất kỳ trong một chuỗi.
...
Đề bài bắt ta tính số lần xóa ít nhất để hai chuỗi trở nên giống nhau, vậy ta có thể nghĩ xem, cuối cùng hai chuỗi này sẽ bị xóa thànhhình dạng gì?
...
Đây là bài 712「Tổng ASCII xóa nhỏ nhất cho hai chuỗi」trên LeetCode, đề tương tự bài trước, chỉ là bài trước yêu cầu tối t hiểu hóa số lần xóa, bài này yêu cầu tối t hiểu hóa tổng mã ASCII của các ký tự bị xóa.
...
**Bài này không thểtái sử dụng trực tiếp hàm tính dãy con chung dài nhất, nhưng có thể làm theo ý tưởng trước đó, sửa nhẹ base case và phần chuyển trạng thái là viết thẳng được code cách giải **:
...
base case có khác biệt nhất định, khi tính độ dài `lcs`, nếu một chuỗi rỗng thì độ dài `lcs` chắc chắn là 0; nhưng bài này nếu một chuỗi rỗng, chuỗi còn lại chắc chắn phải bị xóa toàn bộ, nên cần tính tổng mã ASCII của mọi ký tự trong chuỗi còn lại.

Về chuyển trạng thái, khi `s1[i]` và `s2[j]` giống nhau thì không cần xóa, khi khác nhau thì cần xóa, nên có thể dùng hàm `dp` tính hai trường hợp, đưa ra kết quả tối ưu. Các chỗ khác tương tự nhau, không mở rộng cụ thể nữa.

Tới đây, ba bài toán dãy con đã giải xong, mấu chốt là thu nhỏ bài toán về ký tự, dựa vào mỗi cặp ký tự có giống nhau không để phán đoán chúng có nằm trong dãy con kết quả không, từ đó tránh liệt kê mọi dãy con.

Đây cũng coi là ý tưởng thường dùng khi tìm dãy con trong hai chuỗi, khuyến nghị ngẫm kỹ, luyện tập nhiều~
...
<s trong>Cài [plugin làm bài Chrome của mình](https://labuladong.online/algo/intro/chrome/) rồimở các bài dưới đây để xem thẳng ý tưởng giải:</s trong>
