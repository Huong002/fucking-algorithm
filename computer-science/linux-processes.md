...
Process trong Linux chính là một cấu trúc dữ liệu, hiểu rõ là hiểu được nguyên lý tầng đáy của file descriptor, redirect, lệnh pipe, cuối cùng ta nhìn từ góc OS xem vì sao nói thread và process cơ bản không khác.
...
User space chứa tài nguyên process user cần dùng, ví dụ bạn mở một mảng trong code, mảng này chắc chắn nằm ở user space; kernel space lưu trữ tài nguyên hệ thống mà process kernel cần load, những tài nguyên này thường không cho user truy cập. Nhưng chú ý có process user sẽ share vài tài nguyên kernel space, ví dụ vài thư viện liên kết động v.v.

Ta viết một chương trình hello bằng C, biên dịch được file thực thi, chạy ở dòng lệnh là in ra một câu hello world, rồi chương trình thoát. Ở góc độ OS, chính là tạo mới một process, process này đọc file thực thi ta biên dịch vào không gian bộ nhớ, rồi thực thi, cuối cùng thoát.

**File thực thi bạn biên dịch xong chỉ là một file**, không phải process, file thực thi bắt buộc phải được nạp vào bộ nhớ, đóng gói thành một process mới thực sự chạy được. Process phải nhờ OS tạo, mỗi process đều có thuộc tính vốn có, ví dụ số process (PID), trạng thái process, file đã mở v.v., process tạo xong, đọc vào chương trình của bạn, chương trình của bạn mới được hệ thống thực thi.
...
struct task_struct {
	...
	// Số process
	pid_t			  pid;
	// Con trỏ tới process cha
	struct task_struct __rcu  *parent;
	// Danh sách process con
	struct list_head		children;
	// Con trỏ lưu thông tin filesystem
	struct fs_struct		*fs;
	// Một mảng chứa con trỏ file process này đã mở
	struct files_struct		*files;
};
```

 `task_struct` chính là mô tả của kernel Linux về một process, cũng có thể gọi là 「mô tả process」. Source khá phức tạp, tôi ở đây chỉ trích một phần nhỏ hay gặp.

Trong đó thú vị là con trỏ `mm` và con trỏ `files`. `mm` trỏ tới bộ nhớ ảo của process, chính là nơi nạp tài nguyên và file thực thi; con trỏ `files` trỏ tới một mảng, mảng này chứa mọi con trỏ file process đó đã mở.
...
Ta có thể vẽ lại một hình:

![](https://labuladong.online/algo/images/linuxProcess/2.jpg)

Với máy tính thường, luồng input là bàn phím, luồng output là màn hình, luồng lỗi cũng là màn hình, nên giờ process này nối ba dây với kernel. Vì phần cứng đều do kernel quản, process của ta cần qua 「system call」 để process kernel truy cập tài nguyên phần cứng.
...
Redirect lỗi cũng vậy, không nói dài dòng nữa.

**Ký hiệu pipe** thực ra cũng cùng một ý tưởng, nối output của một process với input của process khác thành một 「pipe」, dữ liệu truyền trong đó, phải nói tư tưởng thiết kế này thực sự rất đẹp:
...
Tới đây, bạn có thể cũng thấy chỗ cao minh của ý tưởng thiết kế 「trong Linux mọi thứ đều là file」, dù là thiết bị, process khác, socket hay file thực sự, toàn bộ đều có thể đọc ghi, thống nhất chứa vào một mảng `files` đơn giản, process qua file descriptor đơn giản truy cập tài nguyên tương ứng, chi tiết cụ thể giao cho OS, tách biệt hiệu quả, đẹp mà hiệu quả.
...
Nói cách khác, thread nhìn không khác process, chỉ là vài vùng dữ liệu của thread share với process cha, còn process con là copy bản sao, chứ không share. Ví dụ như struct `mm` và struct `files` trong thread đều share, tôi vẽ hai hình bạn sẽ hiểu:
...
Vậy bạn có thể hỏi, **đã process và thread na ná, mà đa process dữ liệu không share, tức không tồn tại vấn đề loạn dữ liệu, vì sao dùng đa thread phổ biến hơn đa process nhiều**?

Vì thực tế việc đồng thời share dữ liệu phổ biến hơn mà, ví dụ mười người đồng thời rút mười đồng từ một tài khoản, ta mong số dư tài khoản share này giảm đúng một trăm đồng, chứ không mong mỗi người được một bản copy tài khoản, mỗi bản copy giảm mười đồng.

Đương nhiên, phải nói rõ, chỉ hệ Linux coi thread là process share dữ liệu, không nhìn đặc biệt, nhiều OS khác phân biệt thread và process, thread có cấu trúc dữ liệu đặc hữu, tôi thấy không gọn gàng bằng thiết kế này của Linux, tăng độ phức tạp hệ thống.

Trong Linux tạo thread và process mới hiệu suất đều rất cao, với vấn đề copy vùng nhớ khi tạo process mới, Linux dùng chiến lược copy-on-write để tối ưu, tức không thực sự sao chép không gian bộ nhớ của process cha, mà đợi tới khi cần thao tác ghi mới đi sao chép. **Nên trong Linux tạo process mới và tạo thread mới đều rất nhanh**.
...
<summary><strong>Bài viết trích dẫn bài này</strong></summary>
 ...
 - [Về Linux shell: những mẹo bạn phải biết](https://labuladong.online/algo/fname.html?fname=linuxshell)
